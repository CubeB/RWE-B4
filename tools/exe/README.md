...
| `dis.py <lo> <hi> [--listing F]` | Disassembles one VA range with `objdump -M intel`, address and instruction only, nops dropped. How the `0x2c` bit stream was read routine by routine (builder `0x48B710`, full-state record `0x48B200`, the mover serialisers); grep a full listing for `call   0x415c10` to find every writer of the bit stream, which `xref.py` cannot. |
| `patchdiff.py --base A --other B` | Compares two same-size `TotalA.exe` builds and answers "is this routine patched?". `--range LO:HI` is the test to trust; `--near` is the 512-byte halo test, which crosses function boundaries and over-reports. Reproduces the run and byte counts in [`docs/TA-PATCHES.md`](../../docs/TA-PATCHES.md). |
| `buildloop.py [--build-time N --p N]` | Replays the factory, ground and aircraft build missions through the mission service loop `0x43B7C0`, tick by tick, and prints nanoframe-to-finish for each, with and without a stale COB event pending. It is the check behind [`docs/TOTALA-EXE.md`](../../docs/TOTALA-EXE.md) §101 — why a construction aircraft finishes a tick sooner than a factory would — and needs no binary. |
| `coblist.py <file.cob> [function...]` | Lists the instructions of named functions in a compiled unit script. Mods ship `.cob` rather than `.bos`, and §101 depends on what a builder's `StartBuilding`/`StopBuilding`/`Activate` `set`, so this is how to check it for a data set. |
