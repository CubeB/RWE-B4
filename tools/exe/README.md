...
| `buildloop.py [--build-time N --p N]` | Replays the factory, ground and aircraft build missions through the mission service loop `0x43B7C0`, tick by tick, and prints nanoframe-to-finish for each, with and without a stale COB event pending. It is the check behind [`docs/TOTALA-EXE.md`](../../docs/TOTALA-EXE.md) §107 — why a construction aircraft finishes a tick sooner than a factory would — and needs no binary. |
| `coblist.py <file.cob> [function...]` | Lists the instructions of named functions in a compiled unit script. Mods ship `.cob` rather than `.bos`, and §107 depends on what a builder's `StartBuilding`/`StopBuilding`/`Activate` `set`, so this is how to check it for a data set. |
