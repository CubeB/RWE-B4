...
namespace rwe
{
    ...
    /**
     * How close the target has to be before a bomber's stick starts:
     * `1 + attackrunlength + trunc(falltime * speed)`, the arrival tolerance
     * AirStrike's state 4 hands its goal (0x412394). The fall time is from
     * cruise altitude, not the aircraft's actual height, and the speed is
     * the current one, so the trigger is recomputed every tick.
     */
