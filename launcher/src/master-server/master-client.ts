...
export class MasterClientService {
  ...
  get onCreateGameResponse(): Observable<protocol.CreateGameResponsePayload> {
    return this._onCreateGameReponse;
  }
  ...
  get onGameCreated(): Observable<protocol.GameCreatedEventPayload> {
    return this._onGameCreated;
  }
  get onGameUpdated(): Observable<protocol.GameUpdatedEventPayload> {
    return this._onGameUpdated;
  }
  get onGameDeleted(): Observable<protocol.GameDeletedEventPayload> {
    return this._onGameDeleted;
  }
  ...
  connectToServer(connectionString: string) {
    ...
    this.client.on("connect_error", (e: Error) => {
    ...
    this.client.on(
      protocol.GetGamesResponse,
      (data: protocol.GetGamesResponsePayload) => {
        this._onGetGamesReponse.next(data);
      }
    );

    this.client.on(
      protocol.CreateGameResponse,
      (data: protocol.CreateGameResponsePayload) => {
        this._onCreateGameReponse.next(data);
      }
    );

    this.client.on(
      protocol.GameCreatedEvent,
      (data: protocol.GameCreatedEventPayload) => {
        this._onGameCreated.next(data);
      }
    );

    this.client.on(
      protocol.GameUpdatedEvent,
      (data: protocol.GameUpdatedEventPayload) => {
        this._onGameUpdated.next(data);
      }
    );

    this.client.on(
      protocol.GameDeletedEvent,
      (data: protocol.GameDeletedEventPayload) => {
